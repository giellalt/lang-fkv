...
cat src/morphology/clitics.lexc src/morphology/compounding.lexc  src/morphology/affixes/*lexc src/morphology/stems/*lexc |grep ':.* ;' |cut -d '!' -f1 |cut -d ':' -f1 |sed 's/+/¢+/g' |sed 's/@/¢@/g'|tr '¢' '\n' | tr -d ' ' | tr '#"%' '\n'| egrep '(\+|@)' | egrep -v '^(\+|\+\/\-)$' |sort -u > lexctags
